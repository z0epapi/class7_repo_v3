...
resource "aws_route_table" "public-rtb" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.bamc-igw.id
  }
...
resource "aws_route_table" "private-rtb" {
  ...
  route {
    # cidr_block   = aws_vpc.bamc-vpc.cidr_block # To make sure only VPC traffic is allowed -- #ask Joseph about the error
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.bamc-regional-nat.id
  }
