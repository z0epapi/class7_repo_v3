terraform {
  ...
  #   key    = "state_files/113025/terraform.tfstate" # Path in S3 bucket -- NOTE: This is NOT a file path -- S3 is flat - an Object storage service
  #   region = "af-south-1" # NOTE: can be in a different region provider block below -- can also be in different account
  # }
}

provider "aws" {
  region = "af-south-1" # Region value must be a string
}